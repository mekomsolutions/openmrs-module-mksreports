...
public class MKSReportsManageController {
	 ...
	 * 
	 * @param patientId
	 *            the id of patient whose summary you wish to view
	 * @param summaryId
	 *            the id of the patientsummary you wish to view
	 */
	...
	public void renderPatientHistory(ModelMap model, HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			for (ReportDesign rd : reportService.getAllReportDesigns(false)) {
				if (rd.getName().equals(OutpatientHistoryReportManager.REPORT_DESIGN_NAME)) {
					reportDesign = rd;
				}
			}
	 ...
	 *            A {@link StreamSource} built on the input XML.
	 * @param xslTransformStream
	 *            A {@link StreamSource} built on the XSL style sheet.
	 * @param outStream
	 * @throws Exception
	 */
